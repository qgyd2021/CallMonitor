...
if [ "${environment}" == "sz" ]; then
  ...
  ./${build_dir}/CallMonitor \
  --http_port ${http_port} \
  --asr_event_http_host_port "http://192.168.0.250:8002" \
  --call_monitor_stderrthreshold=0 \
  --call_monitor_log_dir=./logs/ \
  > nohup.out &
  ...
  ./${build_dir}/CallMonitor \
  --http_port ${http_port} \
  --asr_event_http_host_port "http://10.65.254.31:8002" \
  --call_monitor_stderrthreshold=0 \
  --call_monitor_log_dir=./logs/ \
  > nohup.out &
...
elif [ "${environment}" == "my" ]; then
  nohup \
  ./${build_dir}/CallMonitor \
  --http_port ${http_port} \
  --asr_event_http_host_port "http://192.168.0.223:8002" \
  --call_monitor_stderrthreshold=0 \
  --call_monitor_log_dir=./logs/ \
  > nohup.out &
